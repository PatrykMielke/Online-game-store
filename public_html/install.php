...
(12, 1, 'Cowiek maupa', 'Opowieść o Cowieku Maupie, największym zbrodniarzu wojennym, jest stworzoną przez polskich anonów trollpastą. Jej popularność sprawiła, że powstała animowana wersja.\n\n');";
    ...
    if ($result->num_rows > 0) { 
        echo "<div class='container'>Błąd: Podany adres email już istnieje w bazie danych.</div>";
    
        
        $delete = [];
        $delete[] = "DELETE FROM `${prefix}posiadane_programy`";
        $delete[] = "DELETE FROM `${prefix}odpowiedzi_od_supportu`";
        $delete[] = "DELETE FROM `${prefix}wiadomosci_od_uzytkownikow`";
        $delete[] = "DELETE FROM `${prefix}tagi`";
        $delete[] = "DELETE FROM `${prefix}produkty`";
        $delete[] = "DELETE FROM `${prefix}uzytkownicy`";
        ...
        echo '<button><a href="install.php?step=5">Powrot do formularza</button>';
        return;
    }
